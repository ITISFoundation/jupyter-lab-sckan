...
export DOCKER_IMAGE_NAME ?= jupyterlab-sckan
export DOCKER_IMAGE_TAG ?= 0.1.0

OSPARC_DIR:=$(CURDIR)/.osparc

APP_NAME := jupyterlab-sckan
...
replay: .cookiecutterrc ## re-applies cookiecutter
	# Replaying gh:ITISFoundation/cookiecutter-osparc-jupyterlab-service ...
	@cookiecutter --no-input --overwrite-if-exists \
		--config-file=$< \
		--output-dir="$(abspath $(CURDIR)/..)" \
		"gh:ITISFoundation/cookiecutter-osparc-jupyterlab-service"
